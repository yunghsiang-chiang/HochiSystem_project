...
public partial class System_HApplyRecordSame : System.Web.UI.Page
{
    ...
    #region 動態table
    private void ShowITable(string gCourseID, string gHDateRange, string gHOCPlace, string gHArea)
    {
        ...
        string[] gCourseIDArray = gCourseID.Trim('&').Split('&');
        ...
        gCourseCT = gHDateRangeArray.Length;
        ...
        SqlDataReader QueryHCourseName = SQLdatabase.ExecuteReader("select c.HCourseName from OrderList_Merge as a join HCourse as c on a.HCourseID = c.HID LEFT JOIN HMember AS d ON a.HMemberID = d.HID CROSS APPLY SPLIT('" + gCourseID.Replace("&", "") + "', ',') as e WHERE ((left(c.HDateRange,10)<='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)>='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)<='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and left(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "'and right(c.HDateRange,10)>='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "')) " + gHOCPlace + gHArea + " and a.HStatus='1' and a.HItemStatus='1' AND c.HID = e.value GROUP BY c.HCourseName, c.HDateRange ORDER BY c.HDateRange DESC");


        while (QueryHCourseName.Read())
        {
            gHCourseName = gHCourseName + QueryHCourseName["HCourseName"].ToString() + ",";
        }

        string[] gHCourseNameArray = gHCourseName.Trim(',').Split(',');
        ...
        //HE20230930_將HCourseBooking改成OrderList_Merge
        SqlDataReader QueryArea = SQLdatabase.ExecuteReader("select max(a.HAreaID) as HAreaID, max(d.HArea) as HArea from HMember as a join OrderList_Merge as b on a.HID = b.HMemberID join HCourse as c on b.HCourseID = c.HID left join HArea as d on a.HAreaID=d.HID CROSS APPLY SPLIT('" + gCourseID.Replace("&", "") + "', ',') as e WHERE ((left(c.HDateRange,10)<='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)>='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)<='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and left(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "'and right(c.HDateRange,10)>='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "')) " + gHOCPlace + gHArea + " and b.HStatus='1' and b.HItemStatus='1' AND c.HID LIKE e.value GROUP BY a.HAreaID");
        ...
        while (QueryArea.Read())
        {
            ...
            SqlDataReader QueryHMCBCA = SQLdatabase.ExecuteReader("select d.HArea, a.HPeriod, a.HID, a.HUserName, f.HSystemName from HMember as a join OrderList_Merge as b on a.HID = b.HMemberID join HCourse as c on b.HCourseID = c.HID left join HArea as d on a.HAreaID = d.HID LEFT JOIN HSystem AS f ON a.HSystemID =f.HID CROSS APPLY SPLIT('" + gCourseID.Replace("&", "") + "', ',') as e WHERE ((left(c.HDateRange,10)<='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)>='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)<='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and left(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "'and right(c.HDateRange,10)>='" + gHDateRangeArray[1].Trim() + "') or (left(c.HDateRange,10)>='" + gHDateRangeArray[0].Trim() + "' and right(c.HDateRange,10)<='" + gHDateRangeArray[1].Trim() + "')) " + gHOCPlace + gHArea + " and a.HAreaID = '" + QueryArea["HAreaID"].ToString() + "' and b.HStatus='1' and  b.HItemStatus='1'  AND c.HID = e.value GROUP BY d.HArea, a.HPeriod, a.HID, a.HUserName, f.HSystemName ORDER BY  d.HArea ASC, f.HSystemName DESC");
            ...
            while (QueryHMCBCA.Read())
            {
                ...
                if (gCT == 0)
                {
                    ...
                    for (int b = 0; b < gHCourseNameArray.Length; b++)
                    {
                        HCell[gQueryHCourseCT] = new TableCell();
                        //HCell[i].ColumnSpan = 3;
                        HCell[gQueryHCourseCT].Text = gHCourseNameArray[b].ToString();
                ...
                for (int h = 0; h < gCourseIDArray.Length; h++)
                {
                    ...
                    //HE20230930_將HCourseBooking改成OrderList_Merge
                    SqlDataReader QueryHMemberCK = SQLdatabase.ExecuteReader("select c.HID as HCourseID from OrderList_Merge as a join HMember as b on b.HID = a.HMemberID join HCourse as c on a.HCourseID = c.HID left join HArea as d on b.HAreaID = d.HID  CROSS APPLY SPLIT('" + gCourseIDArray[h].Trim(',') + "', ',') as e WHERE HCourseID = e.value and b.HID='" + QueryHMCBCA["HID"].ToString() + "' and a.HStatus='1' and a.HItemStatus='1' GROUP BY c.HID ORDER BY c.HID");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            string gDateS = "", gDateE = "";
            gDateS = DateTime.Now.AddMonths(-1).ToString("yyyy/MM/dd");//今天日期減一個月
            gDateE = DateTime.Now.AddMonths(1).ToString("yyyy/MM/dd");//今天日期加一個月
        }
        ...
        if (!IsPostBack)
        {
            ...
            SqlDataReader QueryHCN = SQLdatabase.ExecuteReader("SELECT HID = (SELECT cast(HID AS NVARCHAR ) + ',' FROM HCourse WHERE HCourseName=a.HCourseName AND HDateRange=a.HDateRange FOR XML PATH('')), HCourseName, HDateRange FROM HCourse as a WHERE HVerifyStatus='2' AND HStatus='1' GROUP BY HCourseName, HDateRange ORDER BY HDateRange DESC");
            while (QueryHCN.Read())
            {
                LBox_HCourseName.Items.Add(new ListItem(QueryHCN["HCourseName"].ToString(), QueryHCN["HID"].ToString()));
            }
            QueryHCN.Close();
        }
        ...
        if (!IsPostBack)
        {
            ...
            dbCmd = new SqlCommand(strSelHA, dbConn);
            SqlDataReader MyQueryHA = dbCmd.ExecuteReader();
    ...
    protected void LBtn_Search_Click(object sender, EventArgs e)
    {
        ...
        if (LBox_HCourseName.SelectedValue == "")
        {
        ...
        string gLBox_HCourseName = "";
        foreach (ListItem LBoxHCourseName in LBox_HCourseName.Items)
        {

            if (LBoxHCourseName.Selected == true)
            {
                gLBox_HCourseName += LBoxHCourseName.Value + "&";
            }
        }

        //判斷課程名稱與日期有無輸入
        if (gLBox_HCourseName.Trim('&').Trim(',') == "" && TB_SearchDate.Text.Trim() == "")
        {
            ShowITable("%", "2000/01/01 - 3000/12/31", gHOCPlace, gHArea);
        }
        else if (gLBox_HCourseName.Trim('&').Trim(',') != "" && TB_SearchDate.Text.Trim() == "")
        {
            ShowITable(gLBox_HCourseName.Trim('&').Trim(','), "2000/01/01 - 3000/12/31", gHOCPlace, gHArea);
        }
        else if (gLBox_HCourseName.Trim('&').Trim(',') == "" && TB_SearchDate.Text.Trim() != "")
        {
            ShowITable("%", TB_SearchDate.Text.Trim(), gHOCPlace, gHArea);
        }
        else if (gLBox_HCourseName.Trim('&').Trim(',') != "" && TB_SearchDate.Text.Trim() != "")
        {
            ShowITable(gLBox_HCourseName.Trim('&').Trim(','), TB_SearchDate.Text.Trim(), gHOCPlace, gHArea);
        }
    ...
    protected void IBtn_ToExcel_Click(object sender, ImageClickEventArgs e)
    {
        ...
        //判斷課程名稱與日期有無輸入
        if (gLBox_HCourseName.Trim(',') == "" && TB_SearchDate.Text.Trim() == "")
        {
            ShowITable("%", "2000/01/01 - 3000/12/31", gHOCPlace, gHArea);
        }
        else if (gLBox_HCourseName.Trim(',') != "" && TB_SearchDate.Text.Trim() == "")
        {
            ShowITable(gLBox_HCourseName.Trim(','), "2000/01/01 - 3000/12/31", gHOCPlace, gHArea);
        }
        else if (gLBox_HCourseName.Trim(',') == "" && TB_SearchDate.Text.Trim() != "")
        {
            ShowITable("%", TB_SearchDate.Text.Trim(), gHOCPlace, gHArea);
        ...
        {
            ShowITable(gLBox_HCourseName.Trim(','), TB_SearchDate.Text.Trim(), gHOCPlace, gHArea);
        }
        ...
        Response.Clear();
        //通知瀏覽器下載檔案
        Response.AddHeader("content-disposition", "attachment;filename=同課程同日期參班記錄分析表.xls");
